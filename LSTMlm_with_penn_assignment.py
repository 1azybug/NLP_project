...
class TextRNNUnit(nn.Module):
    def __init__(self):
        super(TextRNNUnit, self).__init__()
        # self.C = nn.Embedding(n_class, embedding_dim=emb_size)

        '''define the parameter of RNN'''
        '''begin'''
        ##Complete this code
        self.W_ax = nn.Linear(n_hidden, n_hidden)
        self.W_aa = nn.Linear(n_hidden, n_hidden)
...
class TextRNN(nn.Module):
    def __init__(self, num_layers=3):
        ...
        '''begin'''
        ##Complete this code
        self.W_ax = nn.Linear(emb_size, n_hidden)
        self.W_aa = nn.Linear(n_hidden, n_hidden)
        self.b_a = nn.Parameter(torch.ones(n_hidden))
        '''end'''
        ...
        self.layers = nn.ModuleList([TextRNNUnit() for _ in range(num_layers - 1)])
    ...
    def forward(self, X):
        ...
        # y,a_0=self.rnn(X,a_0)
        y = []
        for x in X:
            a_0 = self.tanh(self.W_ax(x) + self.W_aa(a_0) + self.b_a)
            y.append(self.W_h(a_0) + self.b_h)

        # y :[n_step, batch_size, n_hidden] ->(num_layer-1 times) [n_step, batch_size, n_hidden]
        for i in range(self.num_layers - 1):
            y = self.layers[i](y)

        # model_output :[batch_size, n_class]
        model_output = self.W(y[-1]) + self.b  # 交叉熵不用softmax
        '''end'''
...
class MyLSTM(nn.Module):
    def __init__(self, num_layers=2):
        ...
        self.lstm = TextLSTM(emb_size, n_hidden, num_layers=num_layers)
        self.output = nn.Linear(n_hidden, n_class)

    def forward(self, X):
        # X: [batch_size,n_step] -> [batch_size,n_step,emb_size]
        X = self.emb(X)
        y, h, c = self.lstm(X)
        # y: [batch_size,n_step,n_hidden]
...
class TorchLSTM(nn.Module):
    def __init__(self, num_layers=2):
        ...
        self.emb = nn.Embedding(n_class, emb_size)
        self.lstm = nn.LSTM(emb_size, n_hidden, num_layers=num_layers, batch_first=True)
        self.output = nn.Linear(n_hidden, n_class)
...
def train_rnnlm():
    # model = TextRNN(num_layers=2) # this is for RNN
    model = MyLSTM(num_layers=2)  # this is for my LSTM
    # model = TorchLSTM(num_layers=2) # this is for torch.nn.LSTM
    model.to(device)
    print(model)
